...
class UltraFastWaveNoWavesNoBuffer:
    """
    ...
    def __init__(self,
                 n_features,
                 population_size=30,
                 generations=30,  # Total generations since no waves
                 mutation_rate=0.1,
    ...
    def partial_fit(self, X, y):
        ...
        # Adaptive generations based on batch size
        batch_size = len(X)
        if batch_size < 50:
            effective_gens = max(10, self.generations // 3)
        elif batch_size < 150:
            effective_gens = max(20, self.generations // 2)
        else:
            effective_gens = self.generations
