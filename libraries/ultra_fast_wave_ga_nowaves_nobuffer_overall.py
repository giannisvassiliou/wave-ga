"""
Ultra-Fast Standard Online GA with Numba JIT Compilation
This variant uses OVERALL fitness only (no blending) to match StandardOnlineGA
No buffer, no waves - trains directly on incoming data
...
def numba_evaluate_population_overall(weights_3d, X, y, n_classes):
    """
    Fully parallelized population fitness evaluation with OVERALL fitness only.
    No blending - just multiclass balanced accuracy (same as StandardOnlineGA).
...
class UltraFastStandardOnlineGA:
    """
    Ultra-optimized Standard Online GA with Numba JIT compilation.
    Uses OVERALL fitness only (no blending) - same as StandardOnlineGA.
    No buffer, no waves - trains directly on incoming data batches.
    """
    ...
    def _train_generation(self, X, y):
        ...
        self.population_fitness = numba_evaluate_population_overall(
